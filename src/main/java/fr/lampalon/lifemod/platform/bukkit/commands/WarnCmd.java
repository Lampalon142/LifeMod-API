...
public class WarnCmd extends BaseSanctionCmd {
    ...
    protected void onSanctionApplied(ICommandSender sender, OfflinePlayer target, Sanction sanction) {
        String success = fr.lampalon.lifemod.platform.bukkit.LifeMod.getInstance().getLangConfig().getString("sanctions.warn.success")
        ...
        if (target.isOnline()) {
            ...
            String received = lang.getMessage("sanctions.warn.received")
                    .replace("%reason%", sanction.getReason());
            ...
            String title = lang.getMessage("sanctions.warn.title");
            String subtitle = lang.getMessage("sanctions.warn.subtitle").replace("%reason%", sanction.getReason());
