...
public class InfoViewerAction implements IStaffAction {
    ...
    public void onInteractEntity(Player player, PlayerInteractEntityEvent event) {
        ...
        String gmName = LifeMod.getInstance().getLangConfig().getString(gmKey, target.getGameMode().name());
        ...
        player.sendMessage(MessageUtil.formatMessage(LifeMod.getInstance().getLangConfig().getString("mod.items.info.gamemode").replace("%gamemode%", gmName)));
